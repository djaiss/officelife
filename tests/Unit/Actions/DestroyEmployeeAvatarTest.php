...
class DestroyEmployeeAvatarTest extends TestCase
{
    use RefreshDatabase;

    #[Test]
    public function it_removes_the_avatar_and_its_files(): void
    {
        Queue::fake();
        Storage::fake('local');
        ...
        $withAvatar = new UpdateEmployeeAvatar(
            author: $user,
            employee: $employee,
            file: UploadedFile::fake()->image('chili.jpg', 400, 400),
        )->execute();

        $path = (string) $withAvatar->avatar_path;

        $result = new DestroyEmployeeAvatar(
            author: $user->fresh(),
        ...
        $this->assertFalse($result->hasAvatar());
        ...
        Queue::assertPushedOn(
            ...
            callback: fn (LogUserAction $job): bool => $job->action === UserActionEnum::EmployeeAvatarDeleted,
        );
    }

    #[Test]
    public function it_does_nothing_when_there_is_no_avatar(): void
    {
        Queue::fake();
        ...
        $result = new DestroyEmployeeAvatar(author: $user, employee: $employee)->execute();
    ...
    public function it_throws_when_the_author_may_only_change_their_own_avatar(): void
    {
        ...
        new DestroyEmployeeAvatar(author: $author, employee: $oscar)->execute();
    }
    ...
    public function it_throws_when_the_employee_belongs_to_another_company(): void
    {
        ...
        new DestroyEmployeeAvatar(author: $author, employee: $stranger)->execute();
    }
}
