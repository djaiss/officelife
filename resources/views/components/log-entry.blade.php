...
<div class="flex items-center gap-3 border-b border-hairline-soft px-4 py-[13px] last:border-b-0">
  ...
  <div class="min-w-0">
    <p class="flex flex-wrap items-baseline gap-x-[9px] text-[13px]">
    ...
    <p class="mt-[3px] text-[13px] text-body">{{ $log->description }}</p>
  </div>

  <time datetime="{{ $log->created_at->toIso8601String() }}" title="{{ $log->created_at->toDayDateTimeString() }}" class="ml-auto shrink-0 font-mono text-[11.5px] text-muted-soft">
