...
<x-app-layout :title="__('Security')">
  ...
  <x-box :title="__('Two factor authentication')">
    ...
    @if (! $viewModel->usesTwoFactorAuthentication())
      <div class="grid gap-x-9 gap-y-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
        <div class="space-y-2.5 text-sm leading-relaxed text-body">
        ...
        <x-button :href="route('settings.twoFactor.new')" class="max-sm:w-full">{{ __('Turn it on') }}</x-button>
      </div>
    @else
      {{--
        ...
        Both sit beside what they act on until there is no width to sit beside
        it, and then below it, filling the line rather than hugging the right
        hand edge on their own. The same goes for the pair that replaces them.
      --}}
      <div class="space-y-6">
